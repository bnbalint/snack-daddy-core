...
type SnackDaddyCoreService interface {
	...
	UpdateSnacks(ctx echo.Context) error
	...
	UpdateUserSnackRankings(ctx echo.Context) error
}
...
func (server *SnackDaddyEchoServer) registerRoutes() {
	...
	snacks.PUT("", server.UpdateSnacks)
	...
	usersSnackRanking.PUT("", server.UpdateUserSnackRankings)
