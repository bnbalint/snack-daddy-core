...
// Returns:
//
//	200 for successful update, returning newly updated user object
//	415 if the body cannot be correctly parsed into a user object
//	409 for a database key conflict
//	500 for all other errors
func (server *SnackDaddyEchoServer) UpdateUser(ctx echo.Context) error {
	server.Logger.Debug("UpdateUser")
	...
	// save the user to the database
	user, dbError := server.DB.UpdateUser(ctx.Request().Context(), user)
	...
	// return 200, and the created user
	return ctx.JSON(http.StatusOK, user)
